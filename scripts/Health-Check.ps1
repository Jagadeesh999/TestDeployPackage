...
if ($statusResp -match '"loadErrors"\s*:\s*\[\]') {
    ...
    Write-Warning "  ⚠️  Load errors found — review IS server log manually"
}
...
try {
    ...
    if ($logResp -match $Package -and $logResp -imatch "SEVERE|FATAL") {
        Write-Warning "  ⚠️  SEVERE/FATAL entries found for $Package in recent log. Review IS console."
    } else {
        Write-Host "  ✅ No severe errors in recent log for $Package"
    }
} catch {
    Write-Warning "  ⚠️  Could not retrieve log (non-fatal): $_"
}
